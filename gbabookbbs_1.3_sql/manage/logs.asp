...
Sub ItemUse()
	...
	If Len(Keyword) > 0 Then
		sqlwhere = " AND im.username LIKE N'%"& Keyword &"%' OR im.targetusername LIKE N'%"& Keyword &"%'"
		sqlpage = " AND username LIKE N'%"& Keyword &"%' OR targetusername LIKE N'%"& Keyword &"%'"
	End If
	...
	If RecordCount > 0 Then
		...
		strSQL = "SELECT TOP 30 im.itemid, im.tid, im.uid, im.username, im.userip, im.targetuid, im.targetusername, im.operation, im.posttime, i.name, ISNULL(t.title, '') FROM "& TablePre &"itemuselogs im INNER JOIN "& TablePre &"items i ON im.itemid = i.itemid LEFT JOIN "& TablePre &"topics t ON im.tid = t.tid WHERE 1 = 1"& sqlwhere
		If Page > 1 Then
...
Sub Main()
	...
	If Len(Keyword) > 0 Then
		sqlwhere = " AND username LIKE N'%"& Keyword &"%' OR targetusername LIKE N'%"& Keyword &"%'"
	End If
