...
Sub Post()
	...
	If Len(d_BanIP) > 0 Then
		sqladdon = IIF(RQ.AdminGroupID <> 1, " AND username = N'"& RQ.UserName &"'", "")
	...
	If Len(IP(1)) > 0 And Len(IP(2)) > 0 And Len(IP(3)) > 0 And Len(IP(4)) > 0 Then
		...
		If ExistsNum <> 4 Then
			RQ.Execute("INSERT INTO "& TablePre &"banip (ip1, ip2, ip3, ip4, username) VALUES ("& IP(1) &", "& IP(2) &", "& IP(3) &", "& IP(4) &", N'"& RQ.UserName &"')")
		End If
	End If
