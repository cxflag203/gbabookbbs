...
If Action = "useitem" Then
	...
	If PostInfo(3, 0) > 0 And PostInfo(5, 0) = 0 Then
		...
		RQ.Execute("UPDATE "& TablePre &"posts SET usershow = N'"& UserShow &"', ifanonymity = 1 WHERE pid = "& PostID)

		If PostInfo(2, 0) = 1 Then
			RQ.Execute("UPDATE "& TablePre &"topics SET usershow = N'"& UserShow &"', ifanonymity = 1 WHERE tid = "& PostInfo(1, 0))
		End If
	End If
