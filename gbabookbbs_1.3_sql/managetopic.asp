...
Sub SaveTopic()
	...
	If RQ.AllowStickTopic = 1 Then
		...
		RQ.Execute("UPDATE "& TablePre &"topics SET fid = "& NewForumID &", typeid = "& TypeID &", displayorder = "& DisplayOrder &", username = N'"& UserName &"', usershow = N'"& UserShow &"', title = N'"& Title &"', clicks = "& clicks &", types = "& Types &", price = "& Price &", iflocked = "& IfLocked &", ifelite = "& IfElite &", disablemodify = "& DisableModify &" WHERE tid = "& RQ.TopicID)
		...
		End If
	Else
		RQ.Execute("UPDATE "& TablePre &"topics SET fid = "& NewForumID &", typeid = "& TypeID &", username = N'"& UserName &"', usershow = N'"& UserShow &"', title = N'"& Title &"', clicks = "& clicks &", types = "& Types &", price = "& Price &", iflocked = "& IfLocked &", ifelite = "& IfElite &", disablemodify = "& DisableModify &" WHERE tid = "& RQ.TopicID)
	End If
...
Sub UpTopic()
	...
	RQ.Execute("UPDATE "& TablePre &"topics SET lastupdate = GETDATE() WHERE tid = "& RQ.TopicID)
...
Sub DeleteTopic()
	...
	If RQ.F_RecycleBin = 0 Then
		...
		If Topicinfo(7, 0) = 1 Then
			AttachListArray = RQ.Query("SELECT savepath, ifthumb FROM "& TablePre &"attachments WHERE tid = "& RQ.TopicID)
			'删除附件
...
Sub AppraisalTopic()
	...
	RQ.Execute("UPDATE "& TablePre &"topics SET title = N'"& NewTitle &"'"& SqlAddon &" WHERE tid = "& RQ.TopicID)
...
Sub DeletePosts()
	...
	'读取附件
	AttachListArray = RQ.Query("SELECT savepath, ifthumb FROM "& TablePre &"attachments WHERE pid IN("& PostID &") AND tid = "& RQ.TopicID)
	...
	'统计并更新帖子剩余回复数量
	RQ.Execute("UPDATE "& TablePre &"topics SET posts = (SELECT COUNT(pid) - 1 FROM "& TablePre &"posts WHERE tid = "& RQ.TopicID &") WHERE tid = "& RQ.TopicID)
...
Sub UpdatePost()
	...
	If Len(d_AttachID) > 0 Then
		AttachListArray = RQ.Query("SELECT savepath, ifthumb FROM "& TablePre &"attachments WHERE aid IN("& d_AttachID &") AND pid = "& PostID)
		If IsArray(AttachListArray) Then
			For i = 0 To UBound(AttachListArray, 2)
				Call DeleteFile(RQ.Attach_Settings(0) &"/"& AttachListArray(0, i))
	...
	If Request.Form("aid").Count > 0 Then
		For i = 1 To Request.Form("aid").Count
			...
			If AttachID > 0 And Not InStr(","& d_AttachID &",", ","& AttachID &",") > 0 Then
				RQ.Execute("UPDATE "& TablePre &"attachments SET description = N'"& Description &"' WHERE aid = "& AttachID &" AND pid = "& PostID)
			End If
		Next
	End If
	...
	If Request.Form("newaid").Count > 0 Then
		For i = 1 To Request.Form("newaid").Count
			...
			If NewAttachID > 0 Then
				RQ.Execute("UPDATE "& TablePre &"attachments SET tid = "& PostInfo(0, 0) &", pid = "& PostID &", description = N'"& NewDescription &"' WHERE aid = "& NewAttachID)
			End If
		Next
	End If

	'更新回复内容
	RQ.Execute("UPDATE "& TablePre &"posts SET message = N'"& Message &"', ifattachment = (SELECT CASE COUNT(aid) WHEN 0 THEN 0 ELSE 1 END FROM "& TablePre &"attachments WHERE pid = "& PostID &") WHERE pid = "& PostID)

	'更新帖子附件信息
	RQ.Execute("UPDATE "& TablePre &"topics SET ifattachment = (SELECT CASE COUNT(aid) WHEN 0 THEN 0 ELSE 1 END FROM "& TablePre &"attachments WHERE tid = "& PostInfo(0, 0) &") WHERE tid = "& PostInfo(0, 0))
