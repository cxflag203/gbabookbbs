...
If Action = "useitem" Then
	...
	TopicInfo = RQ.Query("SELECT t.fid, t.displayorder, t.uid, t.username, t.lastupdate, t.iftask, ISNULL(tt.expirytime, GETDATE()) FROM "& TablePre &"topics t LEFT JOIN "& TablePre &"topictask tt ON t.tid = tt.tid WHERE t.tid = "& RQ.TopicID &" AND t.displayorder >= 0")

	If Not IsArray(TopicInfo) Then
		Call Confirm("帖子不存在或者还没有通过审核。")
	End If

	FloatTime = DateAdd("h", IntCode(RQ.Item_Settings(2)), Now())
