...
Sub ItemOperation()
	...
	If Request.Form("itemid").Count > 0 Then
		For i = 1 To Request.Form("itemid").Count
			...
			If ItemID > 0 And Len(Name) > 0 Then
				RQ.Execute("UPDATE "& TablePre &"items SET name = N'"& Name &"', available = "& Available &", iflog = "& IfLog &", displayorder = "& DisplayOrder &" WHERE itemid = "& ItemID)
			End If
		Next
	End If
	...
	If Len(CheckContent(New_Name)) > 0 And InArray(Array("member", "topic", "anonymity", "other"), New_Types) And Len(CheckContent(New_Identifier)) > 0 And InArray(Array(0, 1), New_Available) And InArray(Array(0, 1), New_Iflog) Then
		...
		If Not IsArray(ItemInfo) Then
			RQ.Execute("INSERT INTO "& TablePre &"items (name, types, identifier, available, iflog, displayorder) VALUES (N'"& New_Name &"', '"& New_Types &"', '"& New_Identifier &"', "& New_Available &", "& New_Iflog &", "& New_DisplayOrder &")")
		End If
	End If
...
Sub Update()
	...
	RQ.Execute("UPDATE "& TablePre &"items SET name = N'"& Name &"', types = '"& Types &"', identifier = '"& Identifier &"', available = "& Available &", iflog = "& Iflog &", description = N'"& Description &"' WHERE itemid = "& ItemID)
