...
Sub UploadAttach()
	...
	For Each Files In Upload.Files(-1)
		...
		If File.SaveToFile(RQ.Attach_Settings(0) &"/"& SavePath) Then
			RQ.Execute("INSERT INTO "& TablePre &"attachments (uid, filename, filetype, filesize, savepath, ifimage, ifthumb) VALUES ("& RQ.UserID &", N'"& File.LocalName &"', '"& File.Mime &"', "& File.Size &", '"& SavePath &"', "& File.IfImage &", "& File.IfThumb &")")

			AttachID = Conn.Execute("SELECT SCOPE_IDENTITY()")(0)
