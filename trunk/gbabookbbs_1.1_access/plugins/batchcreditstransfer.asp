...
Sub CreditsTransfer()
	...
	'如果有接收成功的用户，则写入异动报告
	If n > 0 Then
		RQ.Execute("INSERT INTO "& TablePre &"logs (uid, username, userip, targetuid, targetusername, operation, reason) SELECT "& RQ.UserID &", '"& RQ.UserName &"', '"& RQ.UserIP &"', uid, username, '"& RQ.Other_Settings(0) &"批转，发放总量为："& CreditsTotalNum &"，平均每人获得"& CreditsTransNum & RQ.Other_Settings(0) &"。', '"& Reason &"' FROM "& TablePre &"members WHERE uid IN("& UserIDs &") AND credits >= "& IntCode(RQ.User_Settings(7)))
	End If

	'写入无法接收金币的用户异动报告
	RQ.Execute("INSERT INTO "& TablePre &"logs (uid, username, userip, targetuid, targetusername, operation, reason) SELECT "& RQ.UserID &", '"& RQ.UserName &"', '"& RQ.UserIP &"', uid, username, '"& RQ.Other_Settings(0) &"批转，发放总量为："& CreditsTotalNum &"，平均每人获得"& CreditsTransNum & RQ.Other_Settings(0) &"。<span class=""pink"">但是由于接收人的"& RQ.Other_Settings(0) &"未达到"& RQ.User_Settings(7) &"，没有接收到。</span>', '"& Reason &"' FROM "& TablePre &"members WHERE uid IN("& UserIDs &") AND credits < "& IntCode(RQ.User_Settings(7)))
