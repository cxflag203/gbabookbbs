...
Sub ClearAllAnonymity()
	...
	If Len(UserName) > 0 Then
		AffectRowCount = RQ.Execute("DELETE FROM "& TablePre &"chatmessages WHERE roomid = "& RoomID &" AND usershow = '"& UserName &"'")

		If AffectRowCount > 0 Then
			strMessage = UserName &"的发言被"& RQ.UserName &"电得无影无踪。"
			RQ.Execute("INSERT INTO "& TablePre &"chatmessages (roomid, usershow, message) VALUES ("& RoomID &", '<b>系统提示</b>', '"& strMessage &"')")
		End If
	End If
...
Sub Dice()
	...
	RQ.Execute("INSERT INTO "& TablePre &"chatmessages (uid, roomid, usershow, message) VALUES ("& RQ.UserID &", "& RoomID &", '<b>系统提示</b>', '"& strMessage &"')")
...
Sub SetAnounceMsg()
	...
	If Len(CheckContent(Message)) > 0 Then
		...
		If RQ.UserCredits >= IntCode(RQ.Chat_Settings(7)) Then
			...
			n = RQ.Execute("UPDATE "& TablePre &"chatannounces SET uid = "& RQ.UserID &", usershow = '"& RQ.UserName &"', message = '"& Message &"' WHERE roomid = "& RoomID)
			If n = 0 Then
				RQ.Execute("INSERT INTO "& TablePre &"chatannounces (roomid, uid, usershow, message) VALUES ("& RoomID &", "& RQ.UserID &", '"& RQ.UserName &"', '"& Message &"')")
			End If

			RQ.Execute("INSERT INTO "& TablePre &"chatmessages (roomid, usershow, message) VALUES ("& RoomID &", '<b>系统提示</b>', '"& RQ.UserName &"将本房间公告设置为："& Message &"')")
		End If
	End If
...
Sub DeleteMsg()
	...
	If RQ.UserCredits > IntCode(RQ.Chat_Settings(5)) And RQ.UserCredits > IntCode(RQ.Chat_Settings(6)) Then
		...
		If IsArray(MessageInfo) Then
			RQ.Execute("UPDATE "& TablePre &"chatmessages SET message = '"& RQ.UserName &"删除了一条发言', usershow = '<b>系统提示</b>' WHERE msgid = "& MessageID)
		End If
	End If
