...
Sub LeagueOp()
	...
	ElseIf Len(Request.Form("btnupdate")) > 0 Then
		LeagueListArray = RQ.Query("SELECT leagueid FROM "& TablePre &"leagues")
		If IsArray(LeagueListArray) Then
			For i = 0 To UBound(LeagueListArray, 2)
				Members = Conn.Execute("SELECT COUNT(joinid) FROM "& TablePre &"leaguemembers WHERE leagueid = "& LeagueListArray(0, i) &" AND groupid > 0")(0)
				News = Conn.Execute("SELECT COUNT(articleid) FROM "& TablePre &"leaguenews WHERE leagueid = "& LeagueListArray(0, i))(0)
				Topics = Conn.Execute("SELECT COUNT(*) FROM "& TablePre &"leaguetopics WHERE leagueid = "& LeagueListArray(0, i))(0)
				RQ.Execute("UPDATE "& TablePre &"leagues SET members = "& Members &", news = "& News &", topics = "& Topics &" WHERE leagueid = "& LeagueListArray(0, i))
			Next
		End If
	End If
...
Sub ChangeMaster()
	...
	Else
		...
		RQ.Execute("INSERT INTO "& TablePre &"leaguemembers (uid, leagueid, groupid, username, designation) VALUES ("& UserInfo(0, 0) &", "& LeagueID &", 1, '"& UserInfo(1, 0) &"', '<strong>联盟盟主</strong>')")
	End If
...
Sub Save()
	...
	If Action = "save" Then
		...
		RQ.Execute("INSERT INTO "& TablePre &"leagues (ifadulting, name, description) VALUES ("& ifAdulting &", '"& Name &"', '"& Description &"')")
		LeagueID = Conn.Execute("SELECT MAX(leagueid) FROM "& TablePre &"leagues")(0)
		RQ.Execute("INSERT INTO "& TablePre &"leaguemembers (uid, leagueid, groupid, username, designation) VALUES ("& UserInfo(0, 0) &", "& LeagueID &", 1, '"& UserInfo(1, 0) &"', '联盟盟主')")

		Call RQ.UpdateLGroupID(UserInfo(0, 0))
	Else
		RQ.Execute("UPDATE "& TablePre &"leagues SET ifadulting = "& ifAdulting &", name = '"& Name &"', description = '"& Description &"' WHERE leagueid = "& LeagueID)
	End If
...
Sub ListMembers()
	...
	If Len(Keyword) > 0 Then
		SqlWhere = SqlWhere &" AND username LIKE '%"& Keyword &"%'"
	End If
...
'========================================================
Sub DeleteMembers()
	Dim LeagueID, LeagueInfo, JoinID, Members
	...
	If Len(JoinID) > 0 Then
		...
		Members = Conn.Execute("SELECT COUNT(joinid) FROM "& TablePre &"leaguemembers WHERE leagueid = "& LeagueID &" AND groupid > 0")(0)
		dbQueryNum = dbQueryNum + 1

		RQ.Execute("UPDATE "& TablePre &"leagues SET members = "& Members &" WHERE leagueid = "& LeagueID)
	End If
...
Sub UpdateNews()
	...
	RQ.Execute("UPDATE "& TablePre &"leaguenews SET title = '"& Title &"', message = '"& Message &"' WHERE articleid = "& ArticleID)
...
'========================================================
Sub DeleteNews()
	Dim LeagueID, LeagueInfo, ArticleID, News
	...
	If Len(ArticleID) > 0 Then
		...
		News = Conn.Execute("SELECT COUNT(articleid) FROM "& TablePre &"leaguenews WHERE leagueid = "& LeagueID)(0)
		dbQueryNum = dbQueryNum + 1

		RQ.Execute("UPDATE "& TablePre &"leagues SET news = "& News &" WHERE leagueid = "& LeagueID)
	End If
...
'========================================================
Sub DeleteTopics()
	Dim LeagueID, LeagueInfo, d_TopicID, Topics
	...
	If Len(d_TopicID) > 0 Then
		...
		Topics = Conn.Execute("SELECT COUNT(*) FROM "& TablePre &"leaguetopics WHERE leagueid = "& LeagueID)(0)
		dbQueryNum = dbQueryNum + 1

		RQ.Execute("UPDATE "& TablePre &"leagues SET topics = "& Topics &" WHERE leagueid = "& LeagueID)
	End If
