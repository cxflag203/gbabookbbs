...
If RecordCount > 0 Then
	...
	strSQL = "SELECT TOP "& RQ.Wap_Settings(3) &" tid, title, clicks, posts, ifelite FROM "& TablePre &"topics WHERE fid = "& RQ.ForumID &" AND displayorder = 0"
	If Page > 1 Then
		strSQL = strSQL &" AND lastupdate < (SELECT MIN(lastupdate) FROM (SELECT TOP "& IntCode(RQ.Wap_Settings(3)) * (Page - 1) &" lastupdate FROM "& TablePre &"topics WHERE fid = "& RQ.ForumID &" AND displayorder = 0 ORDER BY lastupdate DESC) AS tblTemp)"
	End If
...
If RQ.AllowSearch Then
	Call Append("<input type=""text"" name=""keyword"" value="""" format=""M*m"" size=""8"" emptyok=""true"" /><anchor title=""submit"">论坛搜索<go method=""post"" href=""search.asp?action=search""><postfield name=""keyword"" value=""$(keyword)"" /></go></anchor>")
End If
