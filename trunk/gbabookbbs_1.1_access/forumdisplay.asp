...
Sub Show_Topics()
	...
	Dim strSQL, TopicListArray, CountArray
	...
	strSQL = "SELECT TOP "& IntCode(RQ.Topic_Settings(2)) &" tid, typeid, usershow, title, clicks, posts, lastupdate FROM "& TablePre &"topics WHERE fid = "& RQ.ForumID & IIF(typeid > 0, " AND typeid = "& typeid, "") &" AND displayorder = 0"
	If Page > 1 Then
		strSQL = strSQL &" AND lastupdate < (SELECT MIN(lastupdate) FROM (SELECT TOP "& IntCode(RQ.Topic_Settings(2)) * (Page - 1) &" lastupdate FROM "& TablePre &"topics WHERE fid = "& RQ.ForumID & IIF(typeid > 0, " AND typeid = "& typeid, "") &" AND displayorder = 0 ORDER BY lastupdate DESC) AS tblTemp)"
	End If
