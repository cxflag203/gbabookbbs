...
Sub DesignationStuff()
	...
	If Len(Request.Form("usedesignation")) > 0 Then
		...
		RQ.Execute("UPDATE "& TablePre &"memberfields SET designation = '"& Designation &"' WHERE uid = "& RQ.UserID)

	'不使用任何称号
	ElseIf Len(Request.Form("cleardesignation")) > 0 Then
		RQ.Execute("UPDATE "& TablePre &"memberfields SET designation = '' WHERE uid = "& RQ.UserID)
	End If
...
Sub ShowLog()
	...
	RQ.Execute("DELETE FROM "& TablePre &"logs WHERE posttime < #"& DateAdd("d", -IntCode(RQ.Other_Settings(1)), Now()) &"#")
	...
	Else
		...
		strSQL = "SELECT"& SqlTop &" username, userip, targetusername, operation, reason, posttime FROM "& TablePre &"logs WHERE (username LIKE '%"& Keyword &"%' OR targetusername LIKE '%"& Keyword &"%') ORDER BY posttime DESC"
	End If
...
Sub ShowItemLog()
	...
	Else
		SqlTop = IIF(RQ.IsModerator, "", " TOP 50")
		strSQL = "SELECT"& SqlTop &" ml.username, ml.userip, ml.targetusername, ml.num, ml.price, ml.posttime, i.name FROM "& TablePre &"itemmarketlogs ml INNER JOIN "& TablePre &"items i ON ml.itemid = i.itemid WHERE (ml.username LIKE '%"& Keyword &"%' OR ml.targetusername LIKE '%"& Keyword &"%') ORDER BY ml.posttime DESC"
	End If
...
Sub SetBanner()
	...
	RQ.Execute("UPDATE "& TablePre &"settings SET banner = '"& Banner &"'")
