...
function updateStatus(){
	...
	$.ajax({
		url: "ajax/update.php",
		type: "POST",
		data: "status=" + encodeURIComponent(tweet_encode(text)) + "&in_reply_to=" + $("#in_reply_to").val(),
...
$(function(){
	...
	$(".timeline").find("a").attr("rel","nofollow");
});

/**
* 以下是编码相关代码
**/
function tweet_encode(rtext){
	if(config.tweetEncode){
		var rw=twRuneWord(decodeURIComponent(twCookie.getCookie('runes')));
		return rw.encode(rtext);
	}
	return rtext;
}

function tweet_decode(dtext){
	if(config.tweetEncode){
		var rw=twRuneWord(decodeURIComponent(twCookie.getCookie('runes')));
		return rw.decode(dtext);
	}
	return dtext;
}
...
function transmute(){
	if(config.tweetEncode){
		$('.status_word').not('.decoded').each(function(){
			var $this = $(this);
			var rw = twRuneWord(decodeURIComponent(twCookie.getCookie('runes')));
			$this
			.html(rw.decode($this.html().replace(/^\s+|\s+$/g, "")))
			.addClass('decoded');
			showpic_main();
		});
	}
	if(config.searchEncode){
		$query=$('#query');
		if($query.size() == 0)return;
		var rw = twRuneWord(decodeURIComponent(twCookie.getCookie('runes')));
		$query.val(rw.decode($query.val()));
	}
}
...
$(function(){
	...
	$('#login').submit(function(e){
		var $this=$(this);
		var login = twRuneWord(decodeURIComponent(twCookie.getCookie('login_runes')));
		var $encode = $('#encode');
		var $username = $('#username');
		var $password = $('#password');
		var u = $username.val();
		var p = $password.val();
		$username.val(login.encode('臣本车夫，躬耕于长宁'));
		$password.val(login.encode('苟全性命于天朝，不求闻达于网游'));
		$encode.val(login.encode(u+'今失魔兽，临表涕泣，不知所云'+p));
	});

	$('#header_search_form').submit(function(e){
		e.preventDefault();
		var rw = twRuneWord(decodeURIComponent(twCookie.getCookie('runes')));
		var q = $('#header_search_query').val(),eq=q;
		if(config.searchEncode){
			eq = encodeURIComponent(rw.encode(q));
		}
		window.location = 'search.php?q='+eq;
		return false;
	});
	...
	$('#search_form').submit(function(e){
		e.preventDefault();
		var rw = twRuneWord(decodeURIComponent(twCookie.getCookie('runes')));
		var q = $('#query').val(),eq=q;
		var l = $('#l').val();
		if(config.searchEncode){
			eq = encodeURIComponent(rw.encode(q));
		}
		window.location = 'search.php?l='+l+'&q='+eq;
		return false;
	});
