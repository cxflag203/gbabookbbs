...
Sub JoinLeague()
	...
	If LeagueInfo(0, 0) = 1 Then
		RQ.Execute("INSERT INTO "& TablePre &"leaguemembers (uid, leagueid, groupid, username, designation) VALUES ("& RQ.UserID &", "& RQ.LeagueID &", -1, N'"& RQ.UserName &"', N'待审核成员')")
		strTips = "您已经申请加入“"& LeagueInfo(1, 0) &"”，请等待联盟盟主审核。"
	Else
		RQ.Execute("INSERT INTO "& TablePre &"leaguemembers (uid, leagueid, groupid, username, designation) VALUES ("& RQ.UserID &", "& RQ.LeagueID &", 3, N'"& RQ.UserName &"', N'见习成员')")
...
Sub PassMembers(strUserID)
	...
	'更新联盟成员
	RQ.Execute("UPDATE "& TablePre &"leaguemembers SET groupid = 3, designation = N'见习成员' WHERE uid IN("& strUserID &") AND leagueid = "& RQ.LeagueID &" AND groupid = -1")
...
Sub Main()
	...
	If Len(Keyword) = 0 Then
		...
		End If
	Else
		MemberListArray = RQ.Query("SELECT TOP 50 joinid, uid, username, designation FROM "& TablePre &"leaguemembers WHERE leagueid = "& RQ.LeagueID &" AND groupid = 3 AND username LIKE N'%"& Keyword &"%' ORDER BY joinid DESC")
	End If
...
<form name="memberop" method="post" action="?action=memberop">
  <input type="hidden" name="lid" value="<%= RQ.LeagueID %>" />
  <input type="submit" name="btnjoin" value="申请加入" class="button" />
  <input type="submit" name="btnquit" value="退出联盟" onclick="javascript:if(!confirm('您确定退出“<%= LeagueInfo(1, 0) %>”吗？'))return false;" class="button" />
</form>
<p>
<hr color="black" />
<p>
