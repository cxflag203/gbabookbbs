...
Class Cls_Forum
	...
	Public Sub SetLog(TargetUID, TargetUserName, Operation, Reason)
		Execute("INSERT INTO "& TablePre &"logs (targetuid, targetusername, uid, username, userip, operation, reason) VALUES ("& TargetUID &", '"& TargetUserName &"', "& UserID &", '"& UserName &"', '"& UserIP &"', '"& Operation &"', '"& Reason &"')")
	End Sub
	...
	Public Sub SetItemUserLog(ItemID, TargetUID, TargetUserName, Operation)
		Execute("INSERT INTO "& TablePre &"itemuselogs (itemid, tid, uid, username, userip, targetuid, targetusername, operation) VALUES ("& ItemID &", "& TopicID &", "& UserID &", '"& UserName &"', '"& UserIP &"', "& TargetUID &", '"& TargetUserName &"', '"& Operation &"')")
	End Sub
	...
	Public Sub UpdateLGroupID(strUserID)
		Dim MemberListArray, n
		MemberListArray = Query("SELECT m.uid, IIF(lm.groupid IS NULL, 0, lm.groupid) FROM "& TablePre &"members AS m LEFT JOIN [SELECT TOP 1 uid, groupid FROM "& TablePre &"leaguemembers WHERE uid IN("& strUserID &") AND groupid > 0 ORDER BY groupid ASC]. AS lm ON m.uid = lm.uid WHERE m.uid IN("& strUserID &")")
		If IsArray(MemberListArray) Then
			For n = 0 To UBound(MemberListArray, 2)
				Execute("UPDATE "& TablePre &"members SET leaguegid = "& MemberListArray(1, n) &" WHERE uid = "& MemberListArray(0, n))
			Next
		End If
	End Sub
	...
	Public Sub ClearStickTopic()
		...
		TopicListArray = Query("SELECT TOP 1 1 FROM "& TablePre &"topics WHERE tid IN(SELECT tid FROM "& TablePre &"topictask WHERE expirytime < #"& Now() &"#)")

		If IsArray(TopicListArray) Then
			Execute("DELETE FROM "& TablePre &"sticktopics WHERE tid IN(SELECT tid FROM "& TablePre &"topictask WHERE expirytime < #"& Now() &"#)")
			Execute("UPDATE "& TablePre &"topics SET displayorder = 0, iftask = 0 WHERE tid IN(SELECT tid FROM "& TablePre &"topictask WHERE expirytime < #"& Now() &"#)")
			Execute("DELETE FROM "& TablePre &"topictask WHERE expirytime < #"& Now() &"#")
		End If
