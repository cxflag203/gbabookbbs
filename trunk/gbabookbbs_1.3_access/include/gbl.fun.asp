...
Public Sub WarnBack(str)
	Call closeDatabase()
	Response.Write "<script type=""text/javascript"">alert("""& str &""");javascript:history.go(-1);</script>"
	Response.End()
End Sub
...
Public Sub Confirm(str)
	Call closeDatabase()
	Response.Write "<script type=""text/javascript"">alert("""& str &""");window.close();</script>"
	Response.End()
End Sub
...
'过滤引起ACCESS搜索溢出的日文字符
'========================================================
Public Function ConvertJpChar(str)
	Dim AryChar, n
	AryChar = Split("ゴ|ガ|ギ|グ|ゲ|ザ|ジ|ズ|ヅ|デ|ド|ポ|ベ|プ|ビ|パ|ヴ|ボ|ペ|ブ|ピ|バ|ヂ|ダ|ゾ|ゼ|ア", "|")
	For n = 0 To Ubound(AryChar)
		str = Replace(str, AryChar(n), "&#"& AscW(AryChar(n))&";")
	Next
	ConvertJpChar = str
End Function
        ...
        Case 1
			Select Case FilterType
				Case 0
					...
					TempValue = ConvertJpChar(TempValue)
				Case 1
