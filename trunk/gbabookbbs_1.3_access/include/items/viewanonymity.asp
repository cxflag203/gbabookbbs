...
If Action = "useitem" Then
	...
	PostInfo = RQ.Query("SELECT p.tid, p.uid, p.username, p.ifanonymity FROM "& TablePre &"posts p INNER JOIN "& TablePre &"topics t ON p.tid = t.tid WHERE p.pid = "& PostID &" AND p.ifanonymity = 1 AND t.displayorder >= 0")
