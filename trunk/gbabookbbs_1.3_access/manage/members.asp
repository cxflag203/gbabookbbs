...
Sub Delete_Posts()
	Dim UserID, UserInfo
	Dim PostID, PostListArray, AttachListArray
	...
	If Len(PostID) > 0 Then
		PostListArray = RQ.Query("SELECT tid, COUNT(1) FROM "& TablePre &"posts WHERE pid IN("& PostID &") AND iffirst = 0 GROUP BY tid")
		If IsArray(PostListArray) Then
			For i = 0 To UBound(PostListArray, 2)
				RQ.Execute("UPDATE "& TablePre &"topics SET posts = posts - "& PostListArray(1, i) &" WHERE tid = "& PostListArray(0, i))
			Next
...
Sub Permission()
	...
	Else
		If Not IsArray(Application(CacheName &"_usergroup_"& UserInfo(1, 0))) Then
			Call RQ.Reload_UserGroup_Settings(UserInfo(1, 0))
		End If
...
Sub SavePermission()
	...
	UserID = SafeRequest(2, "uid", 0, 0, 0)
	UserInfo = RQ.Query("SELECT m.uid, IIF(a.uid IS NULL, 0, a.uid) FROM "& TablePre &"members m LEFT JOIN "& TablePre &"access a ON m.uid = a.uid WHERE m.uid = "& UserID)
	If Not IsArray(UserInfo) Then
		Call AdminshowTips("用户不存在或者已经被删除。", "")
	End If
...
Sub ListMembers()
	...
	Select Case show
		Case "reguser"
			sqlwhere = " AND m.regtime >= #"& DATEADD("d", -3, Now()) &"#"
			tbCol = "regtime"
		Case "loginuser"
			sqlwhere = " AND m.logintime >= #"& DATEADD("d", -3, Now()) &"#"
			tbCol = "logintime"
		Case "creditstop"
			tbCol = "credits"
		Case "topictop"
		...
		Case Else
			...
			If Len(Query_UserName) > 0 Then
				If Fuzzy_Query = 1 Then
					sqlwhere = sqlwhere &" AND username LIKE '%"& Query_UserName &"%'"
				Else
					sqlwhere = sqlwhere &" AND username = '"& Query_UserName &"'"
				End If
			End If

			If Len(Query_UserIP) > 0 Then
				If Fuzzy_Query = 1 Then
					sqlwhere = sqlwhere &" AND (regip LIKE '%"& Query_UserIP &"%' OR lastloginip LIKE '%"& Query_UserIP &"%' OR loginip LIKE '%"& Query_UserIP &"%')"
				Else
