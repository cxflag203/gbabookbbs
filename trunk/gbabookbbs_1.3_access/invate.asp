...
Sub Buy()
	...
	RQ.Execute("INSERT INTO "& TablePre &"invate (uid, username, invatecode, expirytime) VALUES ("& RQ.UserID &", '"& RQ.UserName &"', '"& Rand(16) &"', #"& DateAdd("d", RQ.InvateExpiryDay, Now()) &"#)")
...
Sub Main()
	...
	If Len(UserName) > 0 Then
		SearchInfo = RQ.Query("SELECT username FROM "& TablePre &"invate WHERE reguid = (SELECT uid FROM "& TablePre &"members WHERE username = '"& UserName &"')")
	End If
