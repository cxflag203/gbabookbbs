...
Sub SaveProfile()
	...
	If IsArray(UserInfo) Then
		RQ.Execute("UPDATE "& TablePre &"memberprofiles SET profile = '"& strProfile &"', province = '"& Province &"', area = '"& Area &"', gender = "& Gender &", birthyear = "& BirthYear &", birthmonth = "& BirthMonth &", birthday = "& Birthday &", constellation = "& Constellation &", ifphoto = "& If_Photo &" WHERE uid = "& RQ.UserID)
	Else
		RQ.Execute("INSERT INTO "& TablePre &"memberprofiles (uid, profile, province, area, gender, birthyear, birthmonth, birthday, constellation, ifphoto) VALUES ("& RQ.UserID &", '"& strProfile &"', '"& Province &"', '"& Area &"', "& Gender &", "& BirthYear &", "& BirthMonth &", "& Birthday &", "& Constellation &", "& If_Photo &")")
	End If
...
Sub Search()
	...
	If Action = "search" Then
		...
		If RecordCount > 0 Then
			...
			strSQL = "SELECT TOP 50 mp.province, mp.area, mp.birthyear, mp.birthmonth, mp.birthday, m.uid, m.username FROM "& TablePre &"memberprofiles mp INNER JOIN "& TablePre &"members m ON mp.uid = m.uid WHERE 1 = 1"& SqlWhere

			If Page > 1 Then
				strSQL = strSQL &" AND mp.uid < (SELECT MIN(uid) FROM (SELECT TOP "& 50 * (Page - 1) &" mp.uid FROM "& TablePre &"memberprofiles mp INNER JOIN "& TablePre &"members m ON mp.uid = m.uid WHERE 1 = 1"& SqlWhere &" ORDER BY mp.uid DESC) AS tblTemp)"
			End If

			strSQL = strSQL &" ORDER BY mp.uid DESC"
...
<%
		If IsArray(MemberListArray) Then
			...
			For i = 0 To UBound(MemberListArray, 2)
				Response.Write IIF(i Mod 2 = 0, "<tr>", "") &"<td width=""33%""><a href=""?uid="& MemberListArray(5, i) &""" title="""& MemberListArray(0, i) & MemberListArray(1, i) &""" onclick=""return shows3(this.href);"">"& MemberListArray(6, i) &"</a> ("& MemberListArray(2, i) &"-"& MemberListArray(3, i) &"-"& MemberListArray(4, i) &")</td>"& IIF(i Mod 2 <> 0, "</tr>", "")
			Next
  ...
  <table border="0" class="profiletd">
    <tr>
      ...
      <td><input type="text" size="20" value="<%= UserInfo(1, 0) %>">
        [<a href="pm.asp?action=send&u=<%= Server.URLEncode(UserInfo(1, 0)) %>" onclick="return shows(this.href);" class="underline">发送传呼</a>]</td>
    </tr>
    <tr>
      <td>自我描述</td>
