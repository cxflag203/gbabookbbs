...
Sub SendPost()
	...
	If InStr(UserName, ",") > 0 Then
		...
		For i = 0 To UBound(TEMP)
			If Len(TEMP(i)) > 0 And Len(TEMP(i)) <= 20 Then
				TargetUserName = TargetUserName & "'"& TEMP(i) &"',"
			End If
		Next
		...
		If Len(TargetUserName) > 0 Then
			EffectRowcount = RQ.Execute("INSERT INTO "& TablePre &"pm (msgfrom, msgfromid, msgtoid, message, posttime) SELECT '"& RQ.UserName &"', "& RQ.UserID &", uid, '"& Message &"', #"& PostTime &"# FROM "& TablePre &"members WHERE username IN("& TargetUserName &")")
		End If
	...
	Else
		...
		RQ.Execute("INSERT INTO "& TablePre &"pm (msgfrom, msgfromid, msgtoid, message, posttime) VALUES ('"& RQ.UserName &"', "& RQ.UserID &", "& UserInfo(0, 0) &", '"& Message &"', #"& PostTime &"#)")
...
Sub Save()
	...
	PmInfo = RQ.Query("SELECT pm.msgfrom, pm.message, pm.remessage, pm.posttime, IIF(pms.pmid IS NULL, 0, pms.pmid) FROM "& TablePre &"pm pm LEFT JOIN "& TablePre &"pms pms ON pm.pmid = pms.pmid WHERE pm.pmid = "& PmID &" AND pm.msgtoid = "& RQ.UserID)
	If Not IsArray(PmInfo) Then
		Call RQ.showTips("传呼不存在或者已经被删除。", "", "")
	End If
...
Sub ShowFavor()
	...
	If RecordCount > 0 Then
		...
		strSQL = "SELECT TOP "& RQ.Topic_Settings(2) &" pmid, message, posttime FROM "& TablePre &"pms WHERE uid = "& RQ.UserID
		If Page > 1 Then
