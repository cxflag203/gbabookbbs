...
Sub Transfer()
	...
	'记录log
	RQ.Execute("INSERT INTO "& TablePre &"itemmarketlogs (uid, username, userip, targetuid, targetusername, itemid, num) VALUES ("& RQ.UserID &", N'"& RQ.UserName &"', '"& RQ.UserIP &"', "& UserInfo(0, 0) &", N'"& UserInfo(1, 0) &"', "& ItemID &", "& Num &")")

	'发送pm通知
	PmMsg = "<strong>系统通知：</strong><p>"& RQ.UserName &"于"& Now() &"向您转让了"& Num &"个“"& ItemName &"”道具。<p><em>如果回复此消息，"& RQ.UserName &"将会收到。</em>"
	RQ.Execute("INSERT INTO "& TablePre &"pm (msgfrom, msgfromid, msgtoid, message) VALUES (N'"& RQ.UserName &"', "& RQ.UserID &", "& UserInfo(0, 0) &", N'"& PmMsg &"')")
...
Sub ForSale()
	...
	Else
		RQ.Execute("INSERT INTO "& TablePre &"itemmarket (itemid, uid, username, price, num) VALUES ("& ItemID &", "& RQ.UserID &", N'"& RQ.UserName &"', "& Price &", "& Num &")")
	End If
...
Sub BuyItem()
	...
	If IsArray(UserInfo) Then
		...
		'记录log
		RQ.Execute("INSERT INTO "& TablePre &"itemmarketlogs (uid, username, userip, targetuid, targetusername, itemid, num, price) VALUES ("& RQ.UserID &", N'"& RQ.UserName &"', '"& RQ.UserIP &"', "& MarketInfo(1, 0) &", N'"& UserInfo(0, 0) &"', "& MarketInfo(0, 0) &", "& BuyNum &", "& MarketInfo(2, 0) * BuyNum &")")
	End If
