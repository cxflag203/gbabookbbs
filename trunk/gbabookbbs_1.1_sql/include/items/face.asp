...
If Action = "useitem" Then
	...
	RQ.Execute("UPDATE "& TablePre &"posts SET usershow = N'"& UserShow &"', ifanonymity = "& rndNumber &" WHERE pid = "& PostID)

	'如果是发帖人则同时也更新帖子作者
	If PostInfo(1, 0) = 1 Then
		RQ.Execute("UPDATE "& TablePre &"topics SET usershow = N'"& UserShow &"', ifanonymity = "& rndNumber &" WHERE tid = "& PostInfo(0, 0))
	End If
