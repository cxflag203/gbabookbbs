...
Sub NewTopic()
	...
	Dim Sig, LeagueJoinID, IfAnonymity, Price, DisplayOrder, IfAttachment
	Dim Special, PollOptions, ValidDate, Multiple, MaxChoices, Visible, n
	Dim Temp_AboutLink, Temp_ImgLink, UserShow, UserInfo, TypeInfo
	Dim Cmd, NewPostID
	...
	Sig = SafeRequest(2, "sig", 0, 0, 0)
	LeagueJoinID = SafeRequest(2, "leaguejoinid", 0, 0, 0)
	IfAnonymity = SafeRequest(2, "ifanonymity", 0, 0, 0)

	'在帖子中显示的名称(匿名、称号、正常)
	If IfAnonymity = 1 And RQ.UserID > 0 And (RQ.UserCredits >= RQ.F_AnonymityNdCredits Or RQ.F_AnonymityNdCredits = 0) Then
	...
	Else
		...
		If RQ.UserID > 0 Then
			...
			If IsArray(UserInfo) Then
				...
				'是否使用签名
				If Sig = 1 And Len(UserInfo(1, 0)) > 0 Then	
	...
	With Cmd
		...
		.CommandType = 4
		.Prepared = True
		.CommandText = TablePre &"sp_newtopic"
		.Parameters.Item("@fid").Value = RQ.ForumID
		.Parameters.Item("@typeid").Value = TypeID
		.Parameters.Item("@displayorder").Value = DisplayOrder
		.Parameters.Item("@uid").Value = RQ.UserID
		.Parameters.Item("@username").Value = RQ.UserName
		.Parameters.Item("@usershow").Value = UserShow
		.Parameters.Item("@title").Value = Title
		.Parameters.Item("@types").Value = Types
		.Parameters.Item("@special").Value = Special
		.Parameters.Item("@price").Value = Price
		.Parameters.Item("@leaguejoinid").Value = LeagueJoinID
		.Parameters.Item("@iflocked").Value = IfLocked
		.Parameters.Item("@ifanonymity").Value = gblAnonymityResult
		.Parameters.Item("@ifattachment").Value = IfAttachment
		.Parameters.Item("@message").Value = Message
		.Parameters.Item("@userip").Value = RQ.UserIP
		.Execute
		RQ.TopicID = .Parameters.Item("@tid")
		NewPostID = .Parameters.Item("@pid")
	End With
	Set Cmd = Nothing
	...
	If Special = 1 Then
		...
		For i = 0 To UBound(PollOptions)
			If Len(CheckContent(PollOptions(i))) > 0 Then
				...
				RQ.Execute("INSERT INTO "& TablePre &"polloptions (tid, title) VALUES ("& RQ.TopicID &", N'"& PollOptions(i) &"')")
			End If
		Next
	End If
	...
	If Request.Form("newaid").Count > 0 Then
		For i = 1 To Request.Form("newaid").Count
			...
			If NewAttachID > 0 Then
				RQ.Execute("UPDATE "& TablePre &"attachments SET tid = "& RQ.TopicID &", pid = "& pid &", description = N'"& NewDescription &"' WHERE aid = "& NewAttachID &" AND uid = "& RQ.UserID)
			End If
		Next
	End If
End Sub
...
Sub NewReply()
	...
	Dim TopicInfo
	Dim Message, Quot_Message, AboutLink, ImgLink, IfAnonymity, Face, Disable_Autowap, Disable_Update, Sig, SendCredits
	...
	With Cmd
		.ActiveConnection = Conn
		.CommandType = 4
		.CommandText = TablePre &"sp_newreply"
		.Prepared = True
		.Parameters.Item("@fid") = RQ.ForumID
		.Parameters.Item("@tid") = RQ.TopicID
		.Parameters.Item("@uid") = RQ.UserID
		.Parameters.Item("@username") = RQ.UserName
		.Parameters.Item("@usershow") = UserShow
		.Parameters.Item("@message") = Message
		.Parameters.Item("@userip") = RQ.UserIP
		.Parameters.Item("@ifanonymity") = gblAnonymityResult
		.Parameters.Item("@ratemark") = SendCredits
		.Parameters.Item("@disable_update") = Disable_Update
		.Parameters.Item("@postfloodctrl") = PostFloodCtrl
		.Parameters.Item("@ifattachment") = IfAttachment
		.Parameters.Item("@t_ifattachment") = TopicInfo(6, 0)
		.Execute
		NewPostID = .Parameters.Item(0)
	End With
	Set Cmd = Nothing
	dbQueryNum = dbQueryNum + 1
...
<form name="newtopic" method="post" action="?action=newtopic" onkeydown="fastpost('btnsubmit');" onsubmit="return validinput(this);">
  ...
  <% If RQ.UserCredits >= RQ.F_AnonymityNdCredits Or RQ.F_AnonymityNdCredits = 0 Then %>
