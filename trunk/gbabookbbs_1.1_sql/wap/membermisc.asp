...
Sub Main()
	...
	'我发表的帖子
	TopicListArray = RQ.Query("SELECT TOP 3 tid, fid, title, clicks, posts, ifelite FROM "& TablePre &"topics WHERE uid = "& RQ.UserID &" ORDER BY lastupdate DESC")
	...
	'我收藏的帖子
	FavorListArray = RQ.Query("SELECT TOP 3 tid, fid, title, clicks, posts, ifelite FROM "& TablePre &"topics WHERE tid IN(SELECT tid FROM "& TablePre &"favorites WHERE uid = "& RQ.UserID &") ORDER BY lastupdate DESC")
	...
	If IsArray(PostListArray) Then
		For i = 0 To UBound(PostListArray, 2)
			PostListArray(2, i) = WapCode(PostListArray(2, i), 20)
