...
Sub Save_Forum()
	...
	Else
		...
		RQ.Execute("UPDATE "& TablePre &"forums SET name = N'"& Forum_Name &"', allowpost = "& F_AllowPost &", adultingpost = "& F_AdultingPost &", showtopictype = "& F_ShowTopicType &", choosetopictype = "& F_ChooseTopicType &", allowpolltopic = "& F_AllowPollTopic &", autoclose = "& F_AutoClose &", recyclebin = "& F_RecycleBin &", visitndcredits = "& F_VisitNdCredits &", postndcredits = "& F_PostNdCredits &", replyndcredits = "& F_ReplyNdCredits &", anonyndmitycredits = "& F_AnonymityNdCredits &", htmlndcredits = "& F_HtmlNdCredits &" WHERE fid = "& ForumID)
...
Sub Update_Moderators()
	...
	If Len(New_Moderator) > 0 Then
		MemberInfo = RQ.Query("SELECT uid, admingroupid FROM "& TablePre &"members WHERE username = N'"& New_Moderator &"'")
	...
	RQ.Execute("UPDATE "& TablePre &"forumfields SET moderators = N'"& strModerators &"' WHERE fid = "& ForumID)
...
Sub Update_TopicTypes()
	...
	If Request.Form("typeid").Count > 0 Then
		For i = 1 To Request.Form("typeid").Count
			...
			If TypeID > 0 And Len(Name) > 0 Then
				RQ.Execute("UPDATE "& TablePre &"topictypes SET name = N'"& Name &"', description = N'"& Description &"', displayorder = "& DisplayOrder &" WHERE typeid = "& TypeID)
			End If
		Next
	End If
	...
	If Len(New_Name) > 0 Then
		New_DisplayOrder = Conn.Execute("SELECT ISNULL(MAX(displayorder), 0) + 1 FROM "& TablePre &"topictypes WHERE fid = "& ForumID)(0)

		RQ.Execute("INSERT INTO "& TablePre &"topictypes (fid, name, description, displayorder) VALUES ("& ForumID &", N'"& New_Name &"', N'"& New_Description &"', "& New_DisplayOrder &")")
	End If
	...
	RQ.Execute("UPDATE "& TablePre &"forumfields SET topictype = N'"& TopicType &"' WHERE fid = "& ForumID)
...
Sub Update_Merge()
	...
	'更新目标版面帖子和回复统计
	RQ.Execute("UPDATE f SET f.topics = (SELECT COUNT(tid) FROM "& TablePre &"topics WHERE fid = f.fid AND displayorder >= 0), posts = (SELECT COUNT(pid) FROM "& TablePre &"posts WHERE tid IN(SELECT tid FROM "& TablePre &"topics WHERE fid = f.fid)) FROM "& TablePre &"forums f WHERE f.fid = "& d_ForumID)

	'更新原版面帖子和回复统计
	RQ.Execute("UPDATE f SET f.topics = (SELECT COUNT(tid) FROM "& TablePre &"topics WHERE fid = f.fid AND displayorder >= 0), posts = (SELECT COUNT(pid) FROM "& TablePre &"posts WHERE tid IN(SELECT tid FROM "& TablePre &"topics WHERE fid = f.fid)) FROM "& TablePre &"forums f WHERE f.fid = "& s_ForumID)
