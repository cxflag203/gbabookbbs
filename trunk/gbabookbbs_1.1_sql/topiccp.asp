...
Sub SaveReport()
	...
	RQ.Execute("INSERT INTO "& TablePre &"pm (msgfrom, msgfromid, msgtoid, message) SELECT N'"& RQ.UserName &"', "& RQ.UserID &", uid, N'"& Message &"' FROM "& TablePre &"members WHERE (uid IN(SELECT uid FROM "& TablePre &"moderators WHERE fid = "& TopicInfo(0, 0) &")) OR (usergroupid IN(1,2))")
...
Sub SaveLeagueElite()
	...
	If blnSaveTopic Then
		...
		If IsArray(PostListArray) Then
			...
			'加入/更新联盟精华帖
			If IsArray(EliteInfo) Then
				RQ.Execute("UPDATE "& TablePre &"leagueelite SET uid = "& TopicInfo(0, 0) &", username = N'"& TopicInfo(1, 0) &"', title = N'"& TopicInfo(2, 0) &"', message = N'"& s &"', lastupdate = '"& TopicInfo(3, 0) &"' WHERE eliteid = "& EliteInfo(0, 0))
			Else
				RQ.Execute("INSERT INTO "& TablePre &"leagueelite (tid, leagueid, uid, username, title, message, lastupdate) VALUES ("& TopicID &", "& RQ.LeagueID &", "& TopicInfo(0, 0) &", N'"& TopicInfo(1, 0) &"', N'"& TopicInfo(2, 0) &"', N'"& s &"', '"& TopicInfo(3, 0) &"')")

				'写入联盟日志
				RQ.Execute("INSERT INTO "& TablePre &"leaguelogs (leagueid, typeid, username, operation) VALUES ("& RQ.LeagueID &", 2, N'"& RQ.UserName &"', N'联盟添加精华帖子:"& TopicID &"("& RQ.UserIP &")')")
			End If
...
Sub SaveLeagueTopic()
	...
	If Len(Request.Form("addtopic")) > 0 Then
		If Not IsArray(LeagueInfo) Then
			...
			'写日志
			RQ.Execute("INSERT INTO "& TablePre &"leaguelogs (leagueid, typeid, username, operation) VALUES ("& RQ.LeagueID &", 1, N'"& RQ.UserName &"', N'联盟添加帖子:"& RQ.TopicID &"("& RQ.UserIP &")')")
		End If
	...
	ElseIf Len(Request.Form("removetopic")) > 0 Then
		If IsArray(LeagueInfo) Then
			...
			'写日志
			RQ.Execute("INSERT INTO "& TablePre &"leaguelogs (leagueid, typeid, username, operation) VALUES ("& RQ.LeagueID &", 3, N'"& RQ.UserName &"', N'联盟去除帖子:"& RQ.TopicID &"("& RQ.UserIP &")')")
		End If
