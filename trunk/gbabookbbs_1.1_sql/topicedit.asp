...
Sub SavePost()
	...
	If Request.Form("aid").Count > 0 Then
		For i = 1 To Request.Form("aid").Count
			...
			If AttachID > 0 And Not InStr(","& d_AttachID &",", ","& AttachID &",") > 0 Then
				RQ.Execute("UPDATE "& TablePre &"attachments SET description = N'"& Description &"' WHERE aid = "& AttachID &" AND uid = "& RQ.UserID)
			End If
		Next
	End If
	...
	If Request.Form("newaid").Count > 0 Then
		For i = 1 To Request.Form("newaid").Count
			...
			If NewAttachID > 0 Then
				RQ.Execute("UPDATE "& TablePre &"attachments SET tid = "& PostInfo(0, 0) &", pid = "& PostID &", description = N'"& NewDescription &"' WHERE aid = "& NewAttachID &" AND uid = "& RQ.UserID)
			End If
		Next
	End If
	...
	'如果修改的是帖子，则更新帖子信息
	If PostInfo(1, 0) = 1 Then
		RQ.Execute("UPDATE "& TablePre &"topics SET displayorder = "& DisplayOrder &", title = N'"& Title &"', iflocked = "& IfLocked &", ifattachment = (SELECT CASE COUNT(aid) WHEN 0 THEN 0 ELSE 1 END FROM "& TablePre &"attachments WHERE tid = "& PostInfo(0, 0) &") WHERE tid = "& PostInfo(0, 0))
	Else
...
Sub DeletePost()
	...
	Else
		...
		'更新版面回复统计
		RQ.Execute("UPDATE "& TablePre &"topics SET posts = (SELECT COUNT(pid) - 1 FROM "& TablePre &"posts WHERE tid = "& PostInfo(0, 0) &"), ifattachment = (SELECT CASE COUNT(aid) WHEN 0 THEN 0 ELSE 1 END FROM "& TablePre &"attachments WHERE tid = "& PostInfo(0, 0) &") WHERE tid = "& PostInfo(0, 0))
	End If
...
'========================================================
Sub Main()
	Dim PostID, PostInfo, AttachListArray
