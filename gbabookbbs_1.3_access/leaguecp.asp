...
Sub SaveNews()
	...
	RQ.Execute("INSERT INTO "& TablePre &"leaguenews (leagueid, uid, username, title, message) VALUES ("& RQ.LeagueID &", "& RQ.UserID &", '"& RQ.UserName &"', '"& Title &"', '"& Message &"')")
...
Sub SaveBatchPm()
	...
	If Target = "forall" Then
		RQ.Execute("INSERT INTO "& TablePre &"pm (msgfrom, msgfromid, msgtoid, message) SELECT '"& RQ.UserName &"', "& RQ.UserID &", uid, '"& Message &"' FROM "& TablePre &"leaguemembers WHERE leagueid = "& RQ.LeagueID &" AND groupid > 0")
	Else
		RQ.Execute("INSERT INTO "& TablePre &"pm (msgfrom, msgfromid, msgtoid, message) SELECT '"& RQ.UserName &"', "& RQ.UserID &", uid, '"& Message &"' FROM "& TablePre &"leaguemembers WHERE leagueid = "& RQ.LeagueID &" AND groupid IN(1,2)")
	End If
...
Sub UpdateDesignation()
	...
	UserInfo = RQ.Query("SELECT lm.joinid FROM "& TablePre &"members m INNER JOIN "& TablePre &"leaguemembers lm ON m.uid = lm.uid WHERE m.username = '"& UserName &"' AND lm.leagueid = "& RQ.LeagueID &" AND lm.groupid > 0")
	...
	RQ.Execute("UPDATE "& TablePre &"leaguemembers SET designation = '"& Designation &"' WHERE joinid = "& UserInfo(0, 0))
...
Sub BatchUpdateDe()
	...
	RQ.Execute("UPDATE "& TablePre &"leaguemembers SET designation = '"& nDesignation &"' WHERE leagueid = "& RQ.LeagueID &" AND designation = '"& oDesignation &"'")
...
Sub ViewMembers()
	...
	If RecordCount > 0 Then
		...
		strSQL = "SELECT TOP 50 uid, username, groupid, jointime FROM "& TablePre &"leaguemembers WHERE leagueid = "& RQ.LeagueID &" AND groupid > 0"
		...
		strSQL = strSQL &" ORDER BY groupid ASC, joinid ASC"
...
Sub UpdateIntro()
	...
	RQ.Execute("UPDATE "& TablePre &"leagues SET ifadulting = "& IfAdulting &", description = '"& Description &"' WHERE leagueid = "& RQ.LeagueID)
...
Sub UpdateName()
	...
	RQ.Execute("UPDATE "& TablePre &"leagues SET name = '"& Name &"' WHERE leagueid = "& RQ.LeagueID)
...
Sub ViewLogs()
	...
	'删除已过期的记录
	RQ.Execute("DELETE FROM "& TablePre &"leaguelogs WHERE posttime < #"& DATEADD("d", -7, Now()) &"#")
