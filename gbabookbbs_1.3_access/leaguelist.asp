...
Sub LeagueFavorite()
	...
	If Len(strLeagueID) > 0 Then
		RQ.Execute("INSERT INTO "& TablePre &"leaguefavorites SELECT "& RQ.UserID &" AS uid, leagueid FROM "& TablePre &"leagues WHERE leagueid IN("& strLeagueID &")")
...
Sub Main()
	...
	j = 0
	LeagueListArray = RQ.Query("SELECT l.leagueid, l.name, IIF(lf.leagueid IS NULL, 0, lf.leagueid) FROM "& TablePre &"leagues l LEFT JOIN (SELECT leagueid FROM "& TablePre &"leaguefavorites WHERE uid = "& RQ.UserID &") lf ON l.leagueid = lf.leagueid ORDER BY l.leagueid ASC")
...
%>
  ...
  <p>
    ...
    目前已预定联盟<%= j %>个,拖出左边底部的页面点"联盟"可以看到所预定联盟的帖子.
  </p>
</form>
<% End If %>
