...
Sub Search()
	...
	SearchInfo = RQ.Query("SELECT searchid, expirytime FROM "& TablePre &"searchindex WHERE searchstring = '"& SearchString &"'")
	...
	If blnUpdateCache Then
		...
		If SearchID > 0 Then
			RQ.Execute("UPDATE "& TablePre &"searchindex SET searchcount = searchcount + 1, recordcount = "& RecordCount &", tid = '"& TopicID &"', expirytime = #"& DATEADD("s", 3600, Now()) &"# WHERE searchid = "& SearchID)
		Else
			RQ.Execute("INSERT INTO "& TablePre &"searchindex (keyword, searchstring, recordcount, tid, expirytime) VALUES ('"& Keyword &"', '"& SearchString &"', "& RecordCount &", '"& TopicID &"', #"& DATEADD("s", 3600, Now()) &"#)")
...
Function Get_UserID(Keyword, SearchString)
	...
	MemberListArray = RQ.Query("SELECT uid FROM "& TablePre &"members WHERE username = '"& Keyword &"'")
	...
	Else
		RQ.Execute("INSERT INTO "& TablePre &"searchindex (keyword, searchstring, recordcount, tid, expirytime) VALUES ('"& Keyword &"', '"& SearchString &"', 0, '0', #"& DATEADD("s", 3600, Now()) &"#)")
		str = "0"
