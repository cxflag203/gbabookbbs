...
TopicInfo = RQ.Query("SELECT fid, displayorder, uid, username, usershow, title, posttime, lastupdate, posts, special, price, leagueid, ifanonymity, iflocked, iftask, ifattachment FROM "& TablePre &"topics WHERE tid = "& RQ.TopicID)
...
If TopicInfo(11, 0) > 0 Then
	Randomize
	LeagueTopicListArray = RQ.Query("SELECT tid, fid, title FROM "& TablePre &"topics WHERE tid IN(SELECT TOP 10 tid FROM "& TablePre &"leaguetopics WHERE leagueid = "& TopicInfo(11, 0) &" AND tid <> "& RQ.TopicID &" ORDER BY RND(-(tid + "& Int((10000 * Rnd) + 1) &")))")
End If
...
Else
	...
	strAddon = " AND uid = "& ViewAuthorID &" AND ifanonymity = 0"
End If
...
'连接sql语句
If Page = 1 Then
	strSQL = "SELECT TOP "& IntCode(RQ.Topic_Settings(4)) + 1 &" pid, iffirst, uid, username, usershow, message, posttime, ifanonymity, ratemark, ifattachment FROM gb_posts WHERE tid = "& RQ.TopicID & strAddon &" ORDER BY posttime ASC"
Else
	strSQL = "SELECT TOP "& RQ.Topic_Settings(4) &" pid, iffirst, uid, username, usershow, message, posttime, ifanonymity, ratemark, ifattachment FROM gb_posts WHERE tid = "& RQ.TopicID & strAddon &" AND posttime > (SELECT MAX(posttime) FROM (SELECT TOP "& IntCode(RQ.Topic_Settings(4)) * (Page - 1) + 1 &" posttime FROM "& TablePre &"posts WHERE tid = "& RQ.TopicID & strAddon &" ORDER BY posttime ASC) AS tblTemp) ORDER BY posttime ASC"
End If
