...
Sub Set_Group()
	...
	If Request.Form("gid").Count > 0 Then
		For i = 1 To Request.Form("gid").Count
			...
			If GroupID > 0 And Len(Name) > 0 Then
				RQ.Execute("UPDATE "& TablePre &"usergroups SET name = '"& Name &"' WHERE gid = "& GroupID)
			End If
		Next
	End If
	...
	If Len(New_Name) > 0 And Len(Types) > 0 Then
		RQ.Execute("INSERT INTO "& TablePre &"usergroups (name, types) VALUES ('"& New_Name &"', '"& Types &"')")
		If Types = "moderator" Then
			GroupID = Conn.Execute("SELECT MAX(gid) FROM "& TablePre &"usergroups")(0)
