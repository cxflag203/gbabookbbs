...
Sub BanUser()
	...
	Dim UserInfo, GroupInfo, ForumListArray, PostListArray, AttachListArray, Topics
	...
	'读取用户当前信息
	UserInfo = RQ.Query("SELECT m.uid, m.admingroupid, m.username, g.name, g.types FROM "& TablePre &"members m INNER JOIN "& TablePre &"usergroups g ON m.usergroupid = g.gid WHERE m.username = '"& UserName &"'")
	...
	If DeletePost = 1 Then
		PostListArray = RQ.Query("SELECT tid, COUNT(1) FROM "& TablePre &"posts WHERE uid = "& UserInfo(0, 0) &" AND iffirst = 0 GROUP BY tid")
