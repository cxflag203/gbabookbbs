...
Sub Post()
	...
	If Len(deleteid) > 0 Then
		RQ.Execute("DELETE FROM "& TablePre &"wordsfilter WHERE id IN("& deleteid &")"& IIF(RQ.AdminGroupID <> 1, " AND username = '"& RQ.UserName &"'", ""))
	End If
	...
	If Request.Form("id").Count > 0 Then
		For i = 1 To Request.Form("id").Count
			...
			If id > 0 And Len(theWord) > 0 And Len(replaceWith) > 0 Then
				RQ.Execute("UPDATE "& TablePre &"wordsfilter SET theword = '"& theWord &"', replacewith = '"& replaceWith &"' WHERE id = "& id & IIF(RQ.AdminGroupID <> 1, " AND username = '"& RQ.UserName &"'", ""))
			End If
		Next
	End If
	...
	If Len(CheckContent(new_theWord)) > 0 And Len(CheckContent(new_replaceWith)) > 0 Then
		RQ.Execute("INSERT INTO "& TablePre &"wordsfilter (theword, replacewith, username) VALUES ('"& new_theWord &"', '"& new_replaceWith &"', '"& RQ.UserName &"')")
	End If
	...
	RQ.Execute("UPDATE "& TablePre &"settings SET wordsfilter = '"& strWordsFilter &"'")
