...
Sub RegInvate()
	...
	If Len(Keyword) > 0 Then
		sqlwhere = " AND reguid IN (SELECT uid FROM "& TablePre &"members WHERE username LIKE '%"& Keyword &"%')"
		sqlpage = " AND inv.reguid IN (SELECT uid FROM "& TablePre &"members WHERE username LIKE '%"& Keyword &"%')"
	End If
